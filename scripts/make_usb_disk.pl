...
my $exfat_start = 2048;
my $fat32_start = 24576;
my $fat32_sectors = 8192;
my $exfat_sectors = $fat32_start - $exfat_start;
...
substr($fat32, 67, 4, le32(0x55534254));
substr($fat32, 71, 11, "USBBOOT    ");
...
my $readme = "Hello from USB FAT32 boot partition.\n";
my $kernel = "This FAT32 sample was read through USB Mass Storage.\n";
my $bootx64 = "TangPingOS can walk USB FAT32 EFI/BOOT.\n";
my $long_name = "USB FAT32 long file names are visible now.\n";
...
substr($root, 96, 64, lfn_entries("usb long filename.txt", "USBLON~1TXT"));
substr($root, 160, 32, short_entry("USBLON~1TXT", 0x20, 8, length($long_name)));
